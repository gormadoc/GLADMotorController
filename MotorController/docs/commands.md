...
## MCode special interactions in IMS
* Pressing Escape aborts all movement with no deceleration. I expect it immediately sends just `\e` but I'm not sure.
* CTRL+C reboots the system, including running startup programs. We probably don't want to replicate this.
...
##### MD (motion mode)
* Returns the last motion command. 
* If a number is entered without any command, the last motion command (according to MD) will be executed with that value.
##### MF
...
* Usage: `hold <motor>`
#### `move`
* Relative increase/decrease on parameter value
* Usage: `move <motor> <parameter> <value>`